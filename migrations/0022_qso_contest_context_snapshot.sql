ALTER TABLE qso_logs
    ADD COLUMN operator_callsign TEXT,
    ADD COLUMN contest_template_id UUID REFERENCES contest_templates(id) ON DELETE RESTRICT,
    ADD COLUMN contest_definition_version INTEGER,
    ADD COLUMN contest_config JSONB NOT NULL DEFAULT '{}';

ALTER TABLE qso_logs
    ADD CONSTRAINT qso_logs_operator_callsign_uppercase
        CHECK (operator_callsign IS NULL OR operator_callsign = upper(operator_callsign)),
    ADD CONSTRAINT qso_logs_definition_version_positive
...
BEGIN
    ...
    END IF;
    IF selected_event.contest_template_id IS NOT NULL THEN
        SELECT definition_version INTO selected_template_version
    ...
    NEW.contest_definition_version = selected_template_version;
