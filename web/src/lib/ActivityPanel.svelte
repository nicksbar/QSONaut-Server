<script lang="ts">
  ...
  import type { ChannelMessage, DiagnosticReport, QsoLog, Station } from './types';
  let { stations, logs, messages, diagnostics, refresh }: { stations: Station[]; logs: QsoLog[]; messages: ChannelMessage[]; diagnostics: DiagnosticReport[]; refresh: () => Promise<void> } = $props();
</script>
...
<section>
  ...
  <div class="section-head"><div><p class="eyebrow">SUPPORT / OPT-IN SNAPSHOTS</p><h2>Radio diagnostics</h2></div><b>{diagnostics.length} reports</b></div>
